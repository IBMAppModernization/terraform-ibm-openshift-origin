...
resource "null_resource" "pre_install_cluster" {
  ...
  provisioner "file" {
    source      = "${var.master_private_ssh_key}"
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "/tmp/scripts/fixup_hosts_file.sh",
      "cat /root/hosts >> /etc/hosts",
      "/tmp/scripts/pre_install_steps.sh ${join(",",concat(var.app_private_ip,var.infra_private_ip,var.master_private_ip))} ${join(",",concat(var.app_host,var.infra_host,var.master_host))} ${join(",",var.master_private_ip)} ${var.domain}"
    ]
  }
}
